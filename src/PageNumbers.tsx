		...
		<nav className={`${className} ${styles.container}`} {...props}>
			...
			{new Array(pad).fill(0).map((_, idx, a) => (
				<button
					...
					key={`${current}-${idx}`}
					className={`${styles.button} ${styles.pad}`}
					onClick={onClick}
					disabled={current <= a.length - idx - 1}
					value={current - (a.length - idx)}
				>
			...
			<span className={`${styles.button} ${styles.total}`}>/&nbsp;{total}</span>
