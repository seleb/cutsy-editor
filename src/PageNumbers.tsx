...
export function PageNumbers({ goto, current = 0, total, className, ...props }: ComponentProps<'nav'> & { goto: (page: number) => void; current?: number; total: number }) {
	...
	return (
		<nav className={`${className} ${styles.container}`} {...props}>
			<button type="button" className={`${styles.button} ${styles.prev}`} onClick={onClick} disabled={current <= 0} value={current - 1} title="Previous page">
				<Icon icon="<" />
			</button>
			<button type="button" className={`${styles.button} ${styles.first}`} onClick={onClick} disabled={current <= pad} value={0} title="First page">
				{1}
			</button>
			...
			<button type="button" className={`${styles.button} ${styles.current}`} onClick={jump} disabled={total <= 1} title="Jump to page">
				{current + 1}
			</button>
			...
			<button type="button" className={`${styles.button} ${styles.last}`} onClick={onClick} disabled={current >= total - pad - 1} value={total - 1} title="Last page">
				{total}
			</button>
			<button type="button" className={`${styles.button} ${styles.next}`} onClick={onClick} disabled={current >= total - 1} value={current + 1} title="Next page">
				<Icon icon=">" />
			</button>
		</nav>
	);
