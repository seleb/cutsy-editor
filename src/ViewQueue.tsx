	...
	return (
		<Page>
			...
			{queue.length > 0 ? (
				<table className={styles.queue}>
					...
					<tbody>
						{queue.map((command, idx) => {
							const time =
								command.command === 'vid_to_img'
									? toDuration(command.time)
									: `${toDuration(command.start)}-${toDuration(
							...
							return (
								<tr key={`${command.input}-${command.output}`}>
									...
									<td>
										<Icon
											title={audio ? 'Sound' : 'Muted'}
											icon={audio ? 'sound' : 'muted'}
										/>
									</td>
				...
				<p>there&apos;s nothing in the queue :)</p>
			)}
		</Page>
	);
}
